...
export default function Video({onGetGesture} : VideoProps) {
  ...
  function predict(){
    if (!recognizer || !webcamRef.current || !webcamRef.current.video || !canvasRef.current || !divRef.current) {
      return;
    }
  ...
  return (
      <div ref={divRef} className="overflow-hidden rounded-xl relative w-full h-full">
        <Webcam  ref={webcamRef} className='overflow-hidden rounded-xl absolute top-0 left-0 w-full h-full'/>
        <canvas ref={canvasRef} className="absolute z-10 top-0 left-0 w-full h-full"></canvas>
      </div>
  )
}
