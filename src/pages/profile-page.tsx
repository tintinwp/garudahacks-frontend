    ...
    <div className='w-full h-full flex flex-col bg-slate-200'>
      <div className="px-16 w-full h-full mt-40">
        <div className="w-full h-full bg-white rounded-t-3xl relative">
          ...
          accept='image/*' type='file' hidden id='image'></input>
          <div className="pt-36 px-8 text-center flex flex-col gap-1 h-full justify-between">
            <div className="">
              <h1 className='text-[#636363] font-bold text-2xl truncate'>{user?.username}</h1>
            ...
            <Button
              ...
              className='mb-8 mt-4'>{user?.type === 'NotGuest' ? 'Logout' : 'Register'}</Button>
          </div>
        </div>
      </div>
    </div>
...
function Box({number, title, icon}: BoxProps){
  return (<div className='flex items-center w-full py-3 px-2 gap-3 rounded-3xl border border-slate-200'>
